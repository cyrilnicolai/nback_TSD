...
#store retro
proNback_blocks <- Nback_blocks %>% # 14691 prospective trials
  filter(order != 1)
...
#15791 --> 1100 retrospective trials
...
# jpeg("DistributionHRRetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
  ggplot(aes(HR)) + 
  facet_grid(.~nback) +
...
# jpeg("DistributionFARetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
  ggplot(aes(FA)) + 
  facet_grid(.~nback) +
  geom_histogram(aes(y = ..count..), position = "dodge", binwidth=0.02, colour = "black" , fill="white") + 
...
# jpeg("DistributionCRRetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
  ggplot(aes(CR)) + 
  facet_grid(.~nback) +
  geom_histogram(aes(y = ..count..), position = "dodge", binwidth=0.02, colour = "black" , fill="white") +
...
# jpeg("DistributionD'RetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
  ggplot(aes(dprime)) + 
  facet_grid(.~nback) +
  geom_histogram(aes(y = ..count..), position = "dodge", binwidth=0.02, colour = "black" , fill="white") + 
...
# jpeg("DistributionHR2RetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
  ggplot(aes(HR)) + 
  facet_grid(.~nback) +
  geom_histogram(aes(y = ..count..), position = "dodge", binwidth=0.02, colour = "black" , fill="white") + 
...
# jpeg("DistributionFA2RRetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
  ggplot(aes(FA)) + 
  facet_grid(.~nback) +
  geom_histogram(aes(y = ..count..), position = "dodge", binwidth=0.02, colour = "black" , fill="white") + 
...
# jpeg("DistributionCR2RetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
  ggplot(aes(CR)) + 
  facet_grid(.~nback) +
  geom_histogram(aes(y = ..count..), position = "dodge", binwidth=0.02, colour = "black" , fill="white") +
...
# jpeg("DistributionD'2RetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
  ggplot(aes(dprime)) + 
  facet_grid(.~nback) +
  geom_histogram(aes(y = ..count..), position = "dodge", binwidth=0.02, colour = "black" , fill="white") + 
...
```{r}
# jpeg("DistribTERetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
...
Nback_blocks <-  Nback_blocks %>%        #873 -> 845 trials
  filter(abs(ZTiming) <= 2 ) %>%
  dplyr::select(-ZTiming) 


# jpeg("DistribTE2retroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
Nback_blocks %>%
  ggplot(aes(timing_error)) + 
  facet_grid(.~duration) +
           geom_histogram(aes(y = ..count..), position = "dodge", binwidth=1,
...
durbinWatsonTest(resid(Md3)) # = 1.95 no autocorrelation, OK
...
lrtest(MB2, MB3)  #p = 0.10 -> complex model (MB3) is not better
...
lrtest(MB4, MB5)  #p = 0.06 -> complex model (MB5) is not better


compare_performance(MB1, MB2, MB3, MB4, MB5) #MB4 and MB5 -> best AIC
...
durbinWatsonTest(resid(MB4)) # = 2 no autocorrelation, OK

check_collinearity(MB4)  #OK

check_normality(MB4) #not OK, but graphically OK, far from being perfect :
...
emmeans(MB4, ~nback)
emmeans(MB4, pairwise ~ duration + nback)
...
lrtest(MRT2, MRT3) #p > 0.05 n.s complex model (MRT3) is not better

MRT4 <- lm(mRT ~ duration * ILI, data = data)
lrtest(MRT2, MRT4) #p > 0.05 n.s complex model (MRT4) is not better

MRT5 <- lm(mRT ~ duration * nback + ILI, data = data)
lrtest(MRT2, MRT5) #p > 0.05 n.s complex model (MRT5) is not better

compare_performance(MRT1, MRT2, MRT3, MRT4) #MRT2 -> best AIC (very close to MRT3 and MRT4)
...
durbinWatsonTest(resid(MRT2)) # = 1.98 no autocorrelation, OK
...
# jpeg("mRTViolinRetroS1.jpg", width = 3000, height = 2500, units = "px",res=600)
...
# jpeg("Violind'RetroS1.jpg", width = 5000, height = 2500, units = "px",res=600)
  plot_grid(title, plot_row, ncol = 1, rel_heights = c(0.1, 1))
# dev.off()

  
  # jpeg("Violind'WMloadRetroS1.jpg", width = 2500, height = 2500, units = "px",res=600)
violindnback
# dev.off()
  
# jpeg("Violind'durationRetroS1.jpg", width = 2500, height = 2500, units = "px",res=600)
violindduration
# dev.off()
```
...
jpeg("ViolinBiasRetroS1.jpg", width = 5000, height = 2500, units = "px",res=600)
  plot_grid(title, plot_row, ncol = 1, rel_heights = c(0.1, 1))
dev.off()
...
jpeg("violinBiasretrointeractionRetroS1.jpg", width = 3000, height = 2500, units = "px",res=600)
...
lrtest(M1, M4) #n.s p = 0.06933 0.05  M4 not significantly better than M1, but we choose this model because it takes the wmload into account (+ better R2)

M5 <-lm(relative_timing ~ dprime + duration, data=data)
lrtest(M4, M5) #n.s p < 0.05  M5 better than M4, similar AIC, we choose M4 (better R2)

M6 <-lm(relative_timing ~ dprime + duration + logbeta, data=data)
lrtest(M4, M6) #n.s p > 0.05  M6 not better than M4

M7 <-lm(relative_timing ~ logbeta + duration * dprime, data=data)
lrtest(M4, M7) #n.s p > 0.05  M7 not better than M4

M8 <-lm(relative_timing ~ logbeta * duration + dprime, data=data)
lrtest(M4, M8) #n.s p > 0.14

M9 <-lm(relative_timing ~ logbeta * duration * dprime, data=data)
lrtest(M4, M9)  #n.s p = 0.0.08 + high correlation
...
check_heteroscedasticity(M4) #not ok with check, but ok graphically with plot()
check_autocorrelation(M4) #OK
check_collinearity(M4) #OK (only low correlation) residuals are independent and not auto-correlated
anova(M4)
...
R1 <- resid(M4) #residuals
F1 <- fitted(M4) #fitted values
...
hist(R1, breaks = 30)
check_normality(M4) #non-normality --> to resolve
...
#determine emmeans
emmeans(M4, "dprime", at = list(dprime = c(0.01, 1, 2, 3, 4)))
emmeans(M4, ~ nback)
```
...
```{r}
# jpeg("d'pointRetroS1.jpg", width = 2500, height = 2500, units = "px",res=600)
Nback_blocks %>%
...
```{r}
# jpeg("violinWMLoadrDERetroS1.jpg", width = 2500, height = 2500, units = "px",res=600)
...
# jpeg("divergingBarLikertRetroS1.jpg", width = 5000, height = 2500, units = "px",res=600)
ggplot() + 
...
# jpeg("divergingBarLikert2RetroS1", width = 3000, height = 2500, units = "px",res=600)
...
m4b = polr(likert ~ duration + ILI + dprime, data, Hess=T)
lrtest(m3,m4b) # p < 0.05* m4b significantly better than m3, but m3 has a better AIC, and nback has a significant main effect on PoT, not d'
...
compare_performance(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14, m15) #m3, m11, m13, m14 --> best AIC
...
exp(cbind(OR = coef(m3), ci = confint(m3))) #logit effects are difficult to interpret --> exp() to have % change
```
...
s <- with(data, summary(as.numeric(likert) ~ duration + ILI + nback, fun=sf))
s
...
# jpeg("PoTJDJretroS1", width = 3000, height = 2500, units = "px",res=600)
Nback_blocks %>% 
  filter( !is.na(likert)) %>%
...
# jpeg("BarslikertRelativeTiming2RetroS1.jpg", width = 4000, height = 2500, units = "px",res=600)
ggplot() + 
