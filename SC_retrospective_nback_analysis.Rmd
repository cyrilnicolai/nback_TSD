...
M4 <-lm(relative_timing ~ duration + dprime, data=data)
...
lrtest(M6, M7) #n.s p > 0.05  M1 better than M7

M8 <-lm(relative_timing ~ logbeta *  dprime, data=data)
lrtest(M6, M8) #n.s p = 0.14

M9 <-lm(relative_timing ~ ILI * dprime, data=data)
lrtest(M6, M9)  #n.s p = 0.11

M9 <-lm(relative_timing ~ nback * dprime, data=data)
lrtest(M6, M9)  #n.s p = 0.11

M10 <-lm(relative_timing ~ duration * logbeta, data=data)
lrtest(M6, M10)  #n.s p = 0.11


compare_performance(M1, M2, M3, M4, M5, M6, M7, M8, M9) # ~equivalent AIC for every model, but better R2 for M9
...
summary(M6)
check_heteroscedasticity(M6) #ok
check_autocorrelation(M6) #OK
check_collinearity(M6) #not relevant, only one term
