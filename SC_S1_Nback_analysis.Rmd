...
MB9 <-lmer(logbeta ~ ILI * nback * duration + paradigm + (1|PID), data=data,  REML=FALSE)
...
MB11 <-lmer(logbeta ~ ILI * nback * duration + paradigm + Session:duration + paradigm:duration +  paradigm:ILI + paradigm:nback + (1|PID), data=data,  REML=FALSE)
lrtest(MB10, MB11) #p < 0.05*** -> More complex model (MB10) not significantly better than MB8 AND SINGULAR

MB12 <-lmer(logbeta ~ ILI * nback * duration + paradigm + Session:duration + paradigm:duration +  paradigm:ILI + paradigm:nback + (nback|PID), data=data,  REML=FALSE)
lrtest(MB11, MB12) #p < 0.05*** 

MB13 <-lmer(logbeta ~ ILI * nback * duration + paradigm + Session:duration + paradigm:duration +  paradigm:ILI + paradigm:nback + (nback + duration|PID), data=data,  REML=FALSE)
...
summary(MB10)
summary(MB11)
summary(MB12) #best model MB12 --> shows significative main effects of ILI, duration, nback and Paradigmn on Bias + interactions between task duration and session
summary(MB13) 



MB12 <-lmer(logbeta ~ ILI * nback * duration + paradigm + Session:duration + paradigm:duration +  paradigm:ILI + paradigm:nback + (nback|PID), data=data,  REML= T)
summary(MB12)
...
lrtest(MRT12, MRT11) #p < 0.05*** -> better model with paradigm as a random effect, but for Session, ILI and nback, p > 0.05 and fit is singular
...
compare_performance(MRT1, MRT2, MRT3, MRT4, MRT5, MRT6, MRT7, MRT8, MRT9, MRT10, MRT12, MRT13) #MRT7, 8 & 9 -> best AIC but 8 and 9 singular
...
lrtest(M13, M14)   # p < 0.05***    ILI, Session and were not significant and the fit was singular
...
lrtest(m5,m3) #p < 0.05***
lrtest(m3,m5) #p < 0.05*** comparable likelihood ratio, but M3 better AIC than m5
...
lrtest(mm1, mm3) #p < 0.05*** Interaction between rDE and Session was found

mm4 = clmm(likert ~ relative_timing * paradigm +(1|PID), lin = "logit", data, Hess=T, nAGQ= 7)
lrtest(mm3, mm4)
